...
export function LoadFooter():void{
    const footerName:string[] = ["Privacy Policy", "Terms of Service"];
    ...
    let footerParagraph = document.createElement('p') as HTMLParagraphElement;
    footerParagraph.className = 'col-md-4 mb-0 text-body-secondary';
    footerParagraph.innerHTML = '&copy; 2024 Company, Inc';
    footer.appendChild(footerParagraph);
    let footerGroup = document.createElement('ul') as HTMLUListElement;
    ...
    for (const elements in footerName) {
        let footerListTerm = document.createElement('li') as HTMLElement;
        footerListTerm.className = "nav-item";
        let footerLink = document.createElement('a') as HTMLAnchorElement;
    ...
    (document.querySelector("footer") as HTMLElement).appendChild(footer);
