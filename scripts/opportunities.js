...
export function DisplayOpportunitiesPage() {
    ...
    let openModal = new window.bootstrap.Modal(document.getElementById("staticBackdrop"));
    /**
...
async function GetOpportunities(query = "") {
    ...
    return await fetch(`./data/opportunities.json`)
        ...
        const filteredData = data.filter(opportunity => opportunity.title.toLowerCase().includes(query.toLowerCase()) ||
