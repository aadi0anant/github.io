 ...
 *
 */
export function DisplayEventPlanningPage(): void {
    console.log("Calling DisplayEventPlanningPage...");
    // Check if the user is logged in
    const user = CheckLogin() as { username: string } | null;
    if (!user) {
     ...
     */
    async function loadEvents(query:string = ""): Promise<void> {
        console.log("loadEvents Function called...");
        // Load events from local storage
        const events: EventData[] = JSON.parse(localStorage.getItem(username + '_events') || '[]');
        ...
        if (events.length === 0) {
            ...
            document.querySelectorAll('.deleteButton').forEach(button => {
                button.addEventListener('click', async function(this: HTMLButtonElement) {
                    const index = (this as HTMLButtonElement).getAttribute('data-index');
     ...
     */
    createButton.addEventListener("click", async (e: Event): Promise<void> => {
        ...
        try {
            ...
            (document.getElementById("eventPlanningForm") as HTMLFormElement).reset();            
